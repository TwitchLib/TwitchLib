...
namespace TwitchLib.Api
{
    public class Subscriptions
    {
        ...
        public class V3 : ApiSection
        {
            ...
            public async Task<Models.v3.Subscriptions.SubscribersResponse> GetSubscribersAsync(string channel, int limit = 25, int offset = 0, Direction direction = Direction.Ascending, string accessToken = null)
            {
                ...
                return await Api.GetGenericAsync<Models.v3.Subscriptions.SubscribersResponse>($"https://api.twitch.tv/kraken/channels/{channel}/subscriptions", getParams, accessToken, ApiVersion.v3).ConfigureAwait(false);
            }
            #endregion
            #region GetAllSubscribers
            public async Task<List<Models.v3.Subscriptions.Subscriber>> GetAllSubscribersAsync(string channel, string accessToken = null)
            {
                Api.Settings.DynamicScopeValidation(AuthScopes.Channel_Subscriptions, accessToken);
                // initial stuffs
                var allSubs = new List<Models.v3.Subscriptions.Subscriber>();
            ...
            public async Task<Models.v3.Subscriptions.Subscriber> ChannelHasUserSubscribedAsync(string channel, string targetUser, string accessToken = null)
            {
                Api.Settings.DynamicScopeValidation(AuthScopes.Channel_Check_Subscription, accessToken);
                try
                {
                    return await Api.GetGenericAsync<Models.v3.Subscriptions.Subscriber>($"https://api.twitch.tv/kraken/channels/{channel}/subscriptions/{targetUser}", null, accessToken, ApiVersion.v3).ConfigureAwait(false);
                }
                catch
                {
                    return null;
            ...
            public async Task<Models.v3.Subscriptions.ChannelSubscription> UserSubscribedToChannelAsync(string user, string targetChannel, string accessToken = null)
            {
                Api.Settings.DynamicScopeValidation(AuthScopes.User_Subscriptions, accessToken);
                try
                {
                    return await Api.GetGenericAsync<Models.v3.Subscriptions.ChannelSubscription>($"https://api.twitch.tv/kraken/users/{user}/subscriptions/{targetChannel}", null, accessToken, ApiVersion.v3).ConfigureAwait(false);
                }
                catch
                {
                    return null;
