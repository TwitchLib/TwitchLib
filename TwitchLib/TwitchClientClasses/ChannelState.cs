...
namespace TwitchLib
{
    ...
    {
        private bool _r9K, _subsOnly, _slowMode;
        ...
        public ChannelState(string ircString)
        {
            ...
            if (ircString.Split(';')[0].Split('=').Length > 1)
                _broadcasterLanguage = ircString.Split(';')[0].Split('=')[1];
            if (ircString.Split(';')[1].Split('=').Length > 1)
                _r9K = ConvertToBool(ircString.Split(';')[1].Split('=')[1]);
            if (ircString.Split(';')[2].Split('=').Length > 1)
                _slowMode = ConvertToBool(ircString.Split(';')[2].Split('=')[1]);
            if (ircString.Split(';')[3].Split('=').Length > 1)
                _subsOnly = ConvertToBool(ircString.Split(';')[3].Split('=')[1]);
