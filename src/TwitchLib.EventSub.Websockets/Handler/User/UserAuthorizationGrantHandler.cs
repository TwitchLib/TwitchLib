...
/// </summary>
public class UserAuthorizationGrantHandler : INotificationHandler
{
    /// <inheritdoc />
    ...
    public void Handle(EventSubWebsocketClient client, string jsonString, JsonSerializerOptions serializerOptions)
    {
        try
        {
            var data = JsonSerializer.Deserialize<EventSubNotification<UserAuthorizationGrant>>(jsonString.AsSpan(), serializerOptions);

            if (data is null)
                throw new InvalidOperationException("Parsed JSON cannot be null!");

            client.RaiseEvent("UserAuthorizationGrant", new UserAuthorizationGrantArgs { Notification = data });
        }
        catch (Exception ex)
        {
