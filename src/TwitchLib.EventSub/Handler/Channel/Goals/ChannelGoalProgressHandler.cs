...
namespace TwitchLib.EventSub.Handler.Channel.Goals;

/// <summary>
/// Handler for 'channel.goal.progress' notifications
/// </summary>
public class ChannelGoalProgressHandler : NotificationHandler, INotificationHandler
{
    /// <inheritdoc />
    public string SubscriptionType => "channel.goal.progress";

    private const string EVENT_NAME = "ChannelGoalProgress";
    ...
    public void Handle(WebsocketClient client, string jsonString, JsonSerializerOptions serializerOptions)
    {
        try
        {
            Handle<ChannelGoalProgress, ChannelGoalProgressArgs>(client, jsonString, serializerOptions, EVENT_NAME);
        }
        catch (Exception ex)
        {
    ...
    public void Handle(WebhooksClient client, System.IO.Stream body, Dictionary<string, string> headers, JsonSerializerOptions serializerOptions)
    {
        try
        {
            Handle<ChannelGoalProgress, ChannelGoalProgressArgs>(client, body, headers, serializerOptions, EVENT_NAME);
        }
        catch (Exception ex)
        {
